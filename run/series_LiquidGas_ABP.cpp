...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fmt::print(stderr, "[series_LiquidGas_ABP] {}\n", ex.what());
        return 1;
    }
    ...
    if (cfg_mgr.config.rank_size != rank_size) {
        fmt::print(stderr, "[series_LiquidGas_ABP] rank size = {} doesn't match config.\n", rank_size);
    ...
    const std::string tag = "series_LiquidGas_ABP";
    ...
    if (rank_idx == 0) {
        ...
        if (!clear_samples || !clear_uk || !clear_density || !clear_pressure) {
            fmt::print(stderr, "[series_LiquidGas_ABP] Failed to initialize CSV outputs in {}.\n",
                       sample_dir.string());
        ...
        fmt::print("[series_LiquidGas_ABP] epsilon_deform = {}, lambda_deform = {}\n",
                   options.epsilon_deform, options.lambda_deform);
        fmt::print("[series_LiquidGas_ABP] Lx_from_cfg = {}, Ly_from_cfg = {}, Lx = {}, Ly = {}\n",
...

    {
        ...
        RankZeroPrint(rank_idx,
                      "[series_LiquidGas_ABP] Waiting for MPI_Barrier before stepping.\n");
        MPI_Barrier(MPI_COMM_WORLD);
        RankZeroPrint(rank_idx,
                      "[series_LiquidGas_ABP] Starting simulation loop.\n");

        for (int step = 0; step < n_steps; ++step) {
            sim.step_single_ABP(step <= 30000);
            ...
            if (step % 100 == 0) {
                RankZeroPrint(rank_idx, "[series_LiquidGas_ABP] Step {}.\n", step);
            }
        }
    }
