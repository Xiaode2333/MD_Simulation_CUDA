...
# Lightweight local runner for tests/run_NPH_test/run_test_NPH.
# Loads the same module stack used on the cluster, activates py3, and runs the
# NPH test binary under mpirun (default: 1 rank to match the test config).
# Usage:
#   scripts/devel_run_test_NPH.sh [config_path] [binary] [-- extra mpirun args...]
# Examples:
#   scripts/devel_run_test_NPH.sh
#   scripts/devel_run_test_NPH.sh tests/run_NPH_test/config.json build/run_NPH_test_run_test_NPH
#   scripts/devel_run_test_NPH.sh tests/run_NPH_test/config.json build/run_NPH_test_run_test_NPH -- --bind-to none
...
CONFIG_PATH="${1:-tests/run_NPH_test/config.json}"
BINARY="${2:-build/run_NPH_test_run_test_NPH}"
shift $(( $# >= 1 ? 1 : 0 ))
shift $(( $# >= 1 ? 1 : 0 ))
...
if [ ! -x "${BINARY}" ]; then
    echo "[ERROR] NPH test binary '${BINARY}' is not executable." >&2
    echo "        Build it first, e.g.:  cmake --build build --target run_NPH_test_run_test_NPH" >&2
    exit 2
fi

MPI_RANKS="${MPI_RANKS:-4}"

echo "Running devel NPH test with:"
