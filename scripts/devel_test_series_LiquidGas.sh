...
# Lightweight local test runner for series_LiquidGas.
...
#
# Usage:
#   scripts/devel_test_series_LiquidGas.sh <base_dir> <ori_config> [series_bin] [D overrides...]
#
# Examples:
#   scripts/devel_test_series_LiquidGas.sh \
#       results/devel_LG_test \
#       results/20251210_LG_series/config.json
#
#   scripts/devel_test_series_LiquidGas.sh \
#       results/devel_LG_test \
#       results/20251210_LG_series/config.json \
#       ./build/run_series_LiquidGas \
...
# Optional explicit binary path, otherwise default to build/run_series_LiquidGas
if [ "$#" -ge 1 ] && [[ "$1" == */run_series_LiquidGas || "$1" == run_series_LiquidGas* ]]; then
    SERIES_BIN="$1"
    shift 1
else
    SERIES_BIN="./build_slurm_tmp/build_39e326fc0919ce49e34f9a3fb7fc842e11759012/run_series_LiquidGas"
fi
...
if [ ! -x "$SERIES_BIN" ]; then
    echo "[ERROR] series_LiquidGas binary '$SERIES_BIN' is not executable." >&2
    echo "        Build it first, e.g.:  cmake --build build --target run_series_LiquidGas" >&2
    exit 2
fi
...
echo "Running devel series_LiquidGas with:"
