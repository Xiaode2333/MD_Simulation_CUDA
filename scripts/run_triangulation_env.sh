#!/bin/bash
#SBATCH --job-name=triangulation_env
...
#SBATCH --mem=16G
#SBATCH --output=./results/triangulation_env_%j.out

module reset
module --force purge

# Load an explicit, dependency-compatible stack.
module load StdEnv
...
# Ensure NumPy is available for CMake checks and Python plotting.
python - << 'EOF'
import numpy
EOF
if [ $? -ne 0 ]; then
    echo "[INFO] NumPy not found in current environment; attempting installation via conda."
    if ! conda install -y numpy; then
        echo "[WARNING] conda install numpy failed; trying pip --user."
        if ! pip install --user numpy; then
            echo "[ERROR] Failed to install NumPy via conda or pip." >&2
            exit 1
        fi
    fi
fi
...
echo "Build complete. Now running triangulation_env test..."

# Executable name is derived from tests/triangulation/triangulation_env.cpp:
# target = triangulation_triangulation_env
srun --cpu-bind=none ./build/triangulation_triangulation_env
