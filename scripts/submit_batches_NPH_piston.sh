...
BASE_ROOT="./results/20260311_NPH_piston"
...
SERIES_BIN="${BUILD_ROOT}/run_series_NPH_piston"
...
if [ ! -f "${BUILD_ROOT}/CMakeCache.txt" ] || [ ! -x "$SERIES_BIN" ]; then
    ...
    cmake --build "$BUILD_ROOT" -j --target run_series_NPH_piston
else
    echo "[INFO] Reusing existing build in '${BUILD_ROOT}' for commit ${GIT_HASH}."
fi

echo "Submitting NPH piston temperature array (T=0.5..1.0) into ${BASE_ROOT}"
sbatch --job-name="nph_piston_20260311" \
    --array=0-5 \
    scripts/run_series_NPH_piston.sh \
    "$BASE_ROOT" \
    "$ORI_CONFIG" \
    "$SERIES_BIN" \
    "${EXTRA_OVERRIDES[@]}"
