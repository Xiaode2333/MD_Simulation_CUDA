...
FileWriter::FileWriter(const std::string& path, bool append){
    ...
    if (append) {
        ...
        if (std::memcmp(header.magic, "PFRAME", 6) != 0 || header.version != 1) {
            std::fclose(fp_);
            throw std::runtime_error("Not a valid PFRAME v1 file");
        }
...
void FileWriter::write_frame(const Particle* data,
                    ...
                    std::uint64_t frame_index)
{
    ...
    int zret = ::compress2(compressed.data(), &dest_len,
                            raw, source_len,
                            Z_BEST_COMPRESSION);
    if (zret != Z_OK) {
    ...
    std::uint64_t compressed_bytes =
        static_cast<std::uint64_t>(dest_len);
    ...
    write_exact(fp_, compressed.data(), static_cast<std::size_t>(compressed_bytes));
