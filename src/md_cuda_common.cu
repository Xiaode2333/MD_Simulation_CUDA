...
__global__ void cal_local_K_kernel(const Particle *__restrict__ particles,
    ...
    if (idx < n_local) {
        ...
        // if (std::isinf(vx) || std::isfinite(vy) || std::isnan(vx) ||
        // std::isnan(vy)){
...
__global__ void local_pressure_tensor_profile_kernel(
    ...
    for (int j = 0; j < n_total; ++j) {
        ...
        if (dr_sq_f > 0.0f && dr_sq_f < rc_sq_f) {
            ...
            int bin_pair =
                    static_cast<int>(pos_norm_mid * static_cast<double>(n_bins_local));
            if (bin_pair < 0) {
                bin_pair = 0;
...
// Implements Euler-Maruyama scheme for stochastic Langevin dynamics:
//   θ^{n+1} = θ^n + √(2·D_θ·Δt)·ξ
//   v⃗ = μ·F⃗_int + v0·(cos θ, sin θ) + √(2·D_r·Δt)·η⃗
//   r⃗^{n+1} = r⃗^n + Δt·v⃗
//
// NOTE: The √Δt scaling for noise is essential for correct SDE discretization!
//       - White noise integral over Δt has variance ∝ Δt
//       - Standard deviation is therefore ∝ √Δt
//       - This ensures ⟨Δr²⟩ = 2·D_r·Δt (correct diffusion)
//       - Using Δt instead of √Δt would give wrong variance scaling!
__global__ void step_ABP_kernel(Particle *particles, int n_local, double dt,
                                                                double Lx, double Ly, double mu, double v0,
                                                                double sqrt_2Dr_dt, double sqrt_2Dth_dt,
                                                                curandState *rng_states) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n_local)
        return;

    // Load RNG state for this particle
    curandState local_state = rng_states[idx];

    // 1. Update orientation with rotational diffusion
    double xi = curand_normal_double(&local_state);
    particles[idx].theta += sqrt_2Dth_dt * xi;

    // Wrap theta to [0, 2π)
    double theta = fmod(particles[idx].theta, 2.0 * M_PI);
    if (theta < 0.0) {
        theta += 2.0 * M_PI;
    }
    particles[idx].theta = theta;

    // 2. Compute self-propulsion direction
    double cos_theta = cos(theta);
    double sin_theta = sin(theta);

    // 3. Generate thermal noise for translational diffusion
    double eta_x = curand_normal_double(&local_state);
    double eta_y = curand_normal_double(&local_state);

    // 4. Compute velocity: v = μ·F + v0·û + √(2·D_r·Δt)·η
    //    Note: acc field stores F/mass from force kernel, so for ABP we use it as
    //    F (in ABP mode, mass is not used in the actual dynamics)
    particles[idx].vel.x =
            mu * particles[idx].acc.x + v0 * cos_theta + sqrt_2Dr_dt * eta_x;
    particles[idx].vel.y =
            mu * particles[idx].acc.y + v0 * sin_theta + sqrt_2Dr_dt * eta_y;
...
// Initialize cuRAND RNG states for ABP simulation
