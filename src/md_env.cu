...
MDSimulation::MDSimulation(MDConfigManager config_manager, MPI_Comm comm) {
    ...
    if (cfg_manager.config.rank_idx == 0) {
        fmt::print("[Rank] {}/{}. Memory allocated.\n", cfg_manager.config.rank_idx,
                             cfg_manager.config.rank_size);
        std::fflush(stdout);
    }
...
MDSimulation::MDSimulation(MDConfigManager config_manager, MPI_Comm comm,
    ...
    if (!loaded_particles.size() == h_particles.size()) {
        RankZeroPrint("[Error] Loaded particle count {} doesn't match "
                                    "n_particles_global {}.\n",
...
MDSimulation::~MDSimulation() {
    // Clean up ABP RNG states if allocated
...
void MDSimulation::init_equilibrium_tracker() {
    ...
    energy_window_sample_count =
            static_cast<std::size_t>(std::max(1, approx_samples));
...
void MDSimulation::append_energy_sample(double U) {
    ...
    if (energy_history_capacity == 0) {
        energy_history_capacity =
                std::max<std::size_t>(2, energy_window_sample_count * 2);
    }
...
MDSimulation::WindowStats
MDSimulation::compute_window_stats(std::size_t start_idx) const {
    ...
    stats.variance =
            var_sum / static_cast<double>(energy_window_sample_count - 1);
    return stats;
}

double
MDSimulation::compute_window_relative_change(std::size_t start_idx) const {
    if (energy_window_sample_count == 0 ||
            energy_history.size() < start_idx + energy_window_sample_count) {
        return 0.0;
    }
...
bool MDSimulation::evaluate_equilibrium(double normalized_sensitivity) {
    ...
    // Two adjacent windows (each 100 units long) must exist before we run the
    // slope + t-test checks.
    ...
    const std::size_t second_start =
            energy_history.size() - energy_window_sample_count;
    ...
    const double slope_first =
            std::abs(compute_window_relative_change(first_start));
    const double slope_second =
            std::abs(compute_window_relative_change(second_start));
    ...
    const double se = std::sqrt(stats_first.variance / sample_count +
                                                            stats_second.variance / sample_count + 1e-18);
    double t_stat = 0.0;
    if (se > 0.0) {
    ...
    const int required_passes =
            std::max(1, static_cast<int>(std::round(kBaseRequiredPasses /
...
void MDSimulation::broadcast_params() {
    ...
    if (world_rank == 0) {
        if (cfg_manager.config.rank_size != 0 &&
            ...
            fmt::print(
                    stderr,
                    "[broadcast_params] Error: config.rank_size={} != world_size={}.\n",
                    cfg_manager.config.rank_size, world_size);
            MPI_Abort(comm, 1);
        }
        cfg_manager.config.rank_size = world_size;
        ...
        if (N_global > 0 && Lx_global > 0.0 && Ly_global > 0.0 &&
        ...
        if (cfg_manager.config.n_cap <= 0) {
            ...
            if (max_density > 0.0) {
                ...
                const double total_area = Lx_global * Ly_global;
                double n_cap_estimate =
                        max_density * 2.0 * total_area / static_cast<double>(world_size);
        ...
        // Use a generous safety factor to handle strong deformations and density
        // inhomogeneities.
        const double sigma_max =
                std::max({cfg_manager.config.SIGMA_AA, cfg_manager.config.SIGMA_AB,
                                    cfg_manager.config.SIGMA_BB});
        int halo_cap = static_cast<int>(sigma_max * cfg_manager.config.cutoff *
                                                                        5.0 / Lx_global * N_global);
        // Increase margin to avoid frequent capacity violations.
        halo_cap *= 4;
        if (halo_cap < 256)
            halo_cap = 256;
        // Density-based halo estimate: halo_cap ≈ max_density * 2 * sigma_max *
        // cutoff * 5.0 * Ly.
        ...
        // Ensure halo buffers are never smaller than the per-rank particle
        // capacity.
    ...
    if (world_rank == 0 && serialized_size > 0) {
        std::copy(serialized_config.begin(), serialized_config.end(),
                            buffer.begin());
    }
    char dummy = 0;
...
void MDSimulation::distribute_particles_h2d() {
    ...
    if (n_cap <= 0) {
        fmt::print(stderr, "[distribute_particles_h2d] n_cap <= 0 on rank {}.\n",
                             rank_idx);
        MPI_Abort(comm, 1);
    }
    ...
    if (rank_idx == 0) {
        if (static_cast<int>(h_particles.size()) < N_global) {
            fmt::print(stderr,
                                 "[distribute_particles_h2d] h_particles size={} < "
                                 "n_particles_global={}.\n",
        ...
        for (int i = 0; i < N_global;
                 ++i) { // This function will only be called by rank 0 at
                                // initialization, so on CPU is fine
        ...
        for (int r = 0; r < rank_size; ++r) {
            ...
            if (count > n_cap) {
                fmt::print(stderr,
                                     "[distribute_particles_h2d] bucket {} has {} particles, "
                                     "exceeds n_cap={}.\n",
                                     r, count, n_cap);
                MPI_Abort(comm, 1);
            }
        ...
        for (int r = 1; r < rank_size; ++r) {
            const int count = static_cast<int>(buckets[r].size());
            if (count > 0) {
                MPI_Send(buckets[r].data(), count * static_cast<int>(sizeof(Particle)),
                                 MPI_BYTE, r, 101, comm);
            }
        }
        ...
        if (count0 > 0) {
            if (static_cast<int>(d_particles.size()) < count0) {
                fmt::print(stderr,
                                     "[distribute_particles_h2d] rank 0 d_particles size={} < "
                                     "n_local={}.\n",
        ...
        if (local_count > n_cap) {
            fmt::print(stderr,
                                 "[distribute_particles_h2d] rank {} received {} particles, "
                                 "exceeds n_cap={}.\n",
        ...
        //         fmt::print(stderr,
        //                    "[distribute_particles_h2d] rank {} d_particles
        //                    size={} < n_local={}.\n", rank_idx,
        //                    d_particles.size(), local_count);
        //         MPI_Abort(comm, 1);
        //     }
        ...
        if (local_count > 0) {
            ...
            if (static_cast<int>(h_particles_local.size()) < local_count) {
                fmt::print(stderr,
                                     "[distribute_particles_h2d] rank {} h_particles_local "
                                     "size={} < local_count={}.\n",
            ...
            MPI_Recv(h_particles_local.data(),
                             local_count * static_cast<int>(sizeof(Particle)), MPI_BYTE, 0,
                             101, comm, MPI_STATUS_IGNORE);

            if (static_cast<int>(d_particles.size()) < local_count) {
                fmt::print(stderr,
                                     "[distribute_particles_h2d] rank {} d_particles size={} < "
                                     "n_local={}.\n",
                                     rank_idx, d_particles.size(), local_count);
                MPI_Abort(comm, 1);
            }
            thrust::copy_n(h_particles_local.begin(), local_count,
                                         d_particles.begin());
        }
    }
...
void MDSimulation::collect_particles_d2h() {
    ...
    if (n_local > 0) {
        Particle *d_ptr = thrust::raw_pointer_cast(d_particles.data());
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(h_particles_local.data(), d_ptr,
    ...
    MPI_Allgather(&n_local_int, 1, MPI_INT, counts_particles.data(), 1, MPI_INT,
                                comm);
    ...
    for (int r = 0; r < rank_size; ++r) {
        recvcounts_bytes[r] =
                counts_particles[r] * static_cast<int>(sizeof(Particle));
    ...
    MPI_Gatherv(sendbuf, send_bytes, MPI_BYTE, recvbuf, recvcounts_bytes.data(),
...
void MDSimulation::update_halo() {
    ...
    MPI_Comm_rank(comm, &world_rank);
    if (world_size != rank_size) {
        fmt::print(stderr, "[update_halo] world_size={} != rank_size={}.\n",
                             world_size, rank_size);
        MPI_Abort(comm, 1);
    }
    if (world_rank != rank_idx) {
        fmt::print(stderr, "[update_halo] world_rank={} != rank_idx={}.\n",
                             world_rank, rank_idx);
        MPI_Abort(comm, 1);
    }
    ...
    if (n_local > n_cap) {
        fmt::print(stderr, "[update_halo] rank {} n_local={} exceeds n_cap={}.\n",
    ...
    const double halo_width =
            cfg_manager.config.cutoff * sigma_max * 1.2; // 1.2 safety factor

    Particle *d_local =
            (n_local > 0) ? thrust::raw_pointer_cast(d_particles.data()) : nullptr;
    ...
    if (send_right_count > halo_right_cap) {
        fmt::print(stderr,
                             "[update_halo] rank {} send_right_count={} exceeds "
                             "halo_right_cap={}.\n",
                             rank_idx, send_right_count, halo_right_cap);
        MPI_Abort(comm, 1);
    }

    // Pack halos on device into d_send_left / d_send_right (persistent device
    // buffers)
    ...
    MPI_Sendrecv(&send_left_count, 1, MPI_INT, left_rank, 0, &recv_right_count, 1,
                             MPI_INT, right_rank, 0, comm, &status);

    // Counts: send right, receive from left (left neighbor's right halo)
    MPI_Sendrecv(&send_right_count, 1, MPI_INT, right_rank, 1, &recv_left_count,
                             1, MPI_INT, left_rank, 1, comm, &status);
    ...
    if (recv_right_count > halo_right_cap) {
        fmt::print(stderr,
                             "[update_halo] rank {} recv_right_count={} exceeds "
                             "halo_right_cap={}.\n",
    ...
    Particle *h_recv_left =
            (recv_left_count > 0) ? h_particles_halo_left.data() : nullptr;
    Particle *h_recv_right =
            (recv_right_count > 0) ? h_particles_halo_right.data() : nullptr;
    ...
    if (recv_left_count > 0) {
        CUDA_CHECK(
                cudaMemcpy(thrust::raw_pointer_cast(d_particles_halo_left.data()),
    ...
    if (recv_right_count > 0) {
        CUDA_CHECK(
                cudaMemcpy(thrust::raw_pointer_cast(d_particles_halo_right.data()),
...
void MDSimulation::update_d_particles() { // use only d_particles to update
                                                                                    // particles through particle exchange
                                                                                    // between ranks
    const int rank_idx = cfg_manager.config.rank_idx;
    const int rank_size = cfg_manager.config.rank_size;
    ...
    if (world_size != rank_size) {
        fmt::print(stderr, "[update_d_particles] world_size={} != rank_size={}.\n",
                             world_size, rank_size);
        MPI_Abort(comm, 1);
    }
    if (world_rank != rank_idx) {
        fmt::print(stderr, "[update_d_particles] world_rank={} != rank_idx={}.\n",
                             world_rank, rank_idx);
        MPI_Abort(comm, 1);
    }
    ...
    if (n_local > 0) {
        ...
        if (keep_count + send_left_count + send_right_count != n_local) {
            fmt::print(stderr,
                                 "[update_d_particles] rank {} mismatch: keep={} left={} "
                                 "right={} n_local={}.\n",
        ...
        // No local particles, so nothing to mark; we may still receive from
        // neighbors.
        send_left_count = 0;
        send_right_count = 0;
        keep_count = 0;
    ...
    if (n_new_local > n_cap) {
        fmt::print(stderr,
                             "[update_d_particles] rank {} n_new_local={} exceeds n_cap={} "
                             "(keep={} recvL={} recvR={}).\n",
    ...
    if (recv_left_count > 0) {
        CUDA_CHECK(
                cudaMemcpy(d_recv_left, h_recv_left,
                                     static_cast<size_t>(recv_left_count) * sizeof(Particle),
                                     cudaMemcpyHostToDevice));
    }
    if (recv_right_count > 0) {
...
void MDSimulation::init_particles() {
    ...
    if (rank_idx == 0) {
        ...
        double spacing_type0 =
                std::sqrt(unshifted_x_devide * box_h / n_particles_type0);
        double spacing_type1 =
                std::sqrt((box_w - unshifted_x_devide) * box_h / n_particles_type1);

        // helper to adjust spacing and grid so rows*cols >= N, without "+1" and
        // without wrap
        auto adjust_grid = [](double width, double height, int n_particles,
                                                    double &spacing, int &n_cols, int &n_rows) {
            // initial estimate
        ...
        adjust_grid(unshifted_x_devide, box_h, n_particles_type0, spacing_type0,
                                n_cols_type0, n_rows_type0);

        adjust_grid(box_w - unshifted_x_devide, box_h, n_particles_type1,
        ...
        double total_mass =
                n_particles_type0 * mass_type0 + n_particles_type1 * mass_type1;
        double sum_px = 0.0;
        double sum_py = 0.0;
        ...
        double target_K =
                (2 * static_cast<double>(n_particles_global) - 2) * 0.5 * T_init;
        for (int i = 0; i < n_particles_global; ++i) {
            double mass = (h_particles[i].type == 0) ? mass_type0 : mass_type1;
            current_K += 0.5 * mass *
                                     (h_particles[i].vel.x * h_particles[i].vel.x +
        ...
        // Initialize theta field for ABP (random orientation)
        std::uniform_real_distribution<double> theta_dist(0.0, 2.0 * M_PI);
        for (int i = 0; i < n_particles_global; ++i) {
            h_particles[i].theta = theta_dist(rng);
        }
    }
...
void MDSimulation::plot_particles(const std::string &filename,
    ...
    plot_particles_python(
            h_particles, filename, csv_path, cfg_manager.config.box_w_global,
...
void MDSimulation::middle_reflect_LG() {
    ...
    const double sigma_max =
            std::max({cfg_manager.config.SIGMA_AA, cfg_manager.config.SIGMA_BB,
    ...
    if (n_local > 0 && !d_particles.empty()) {
        ...
        middle_reflect_LG_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles.data()), n_local, Lx, Ly,
                slab_width);
    }
    ...
    if (n_left > 0 && !d_particles_halo_left.empty()) {
        ...
        middle_reflect_LG_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles_halo_left.data()), n_left, Lx, Ly,
                slab_width);
    }
    ...
    if (n_right > 0 && !d_particles_halo_right.empty()) {
        ...
        middle_reflect_LG_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles_halo_right.data()), n_right, Lx,
                Ly, slab_width);
    }
...
void MDSimulation::step_single_NVE() {
    ...
    if (n_local > 0) {
        ...
        step_half_vv_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles.data()), n_local, dt, Lx, Ly);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    ...
    if (n_local > n_cap) {
        fmt::print(stderr,
                             "[step_single_NVE] rank {} n_local={} exceeds n_cap={} after "
                             "exchange.\n",
                             cfg_manager.config.rank_idx, n_local, n_cap);
        MPI_Abort(comm, 1);
    }
...
void MDSimulation::step_single_nose_hoover(bool do_middle_wrap) {
    ...
    if (n_local > n_cap) {
        fmt::print(stderr,
                             "[step_single_nose_hoover] rank {} n_local={} exceeds n_cap={} "
                             "before step.\n",
                             cfg_manager.config.rank_idx, n_local, n_cap);
        MPI_Abort(comm, 1);
    }
    ...
    if (n_local > 0) {
        ...
        step_half_vv_nh_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles.data()), n_local, dt, xi, Lx, Ly);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    ...
    if (n_local > 0) {
        ...
        step_2nd_half_vv_nh_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles.data()), n_local, dt, xi);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    K_global = cal_total_K();
...
void MDSimulation::step_single_ABP(bool do_middle_wrap) {
    ...
    const double v0 = cfg_manager.config.v0;
    const double D_r = cfg_manager.config.D_r;
    const double D_theta = cfg_manager.config.D_theta;

    // Precompute noise scale factors for Euler-Maruyama SDE integration
    // The √Δt scaling (not Δt!) is critical for correct stochastic dynamics:
    //   - Variance of noise must scale as Δt (diffusion ⟨Δr²⟩ = 2·D·Δt)
    //   - Standard deviation therefore scales as √Δt
    //   - η ~ N(0,1) is multiplied by √(2·D·Δt) to get correct variance
    const double sqrt_2Dr_dt = std::sqrt(2.0 * D_r * dt);
    const double sqrt_2Dth_dt = std::sqrt(2.0 * D_theta * dt);

    // Initialize RNG states on first call (lazy initialization)
    ...
    if (n_local > 0 && d_rng_states != nullptr) {
        ...
        step_ABP_kernel<<<blocks, threads>>>(
                thrust::raw_pointer_cast(d_particles.data()), n_local, dt, Lx, Ly, mu,
                v0, sqrt_2Dr_dt, sqrt_2Dth_dt, d_rng_states);
...
double MDSimulation::compute_kinetic_energy_local() {
    ...
    cal_local_K_kernel<<<blocks, threads,
...
double MDSimulation::cal_partial_U_lambda(double epsilon_lambda) {
    ...
    if (n_local > 0) {
        ...
        cal_partial_U_lambda_kernel<<<blocks, threads,
                                                                    threads *static_cast<int>(sizeof(double))>>>(
                thrust::raw_pointer_cast(d_particles.data()),
                thrust::raw_pointer_cast(d_particles_halo_left.data()),
...
double MDSimulation::deform(double epsilon, double U_old) {
    ...
    if (world_size != cfg_manager.config.rank_size) {
        fmt::print(stderr, "[deform] world_size={} != rank_size={}.\n", world_size,
                             cfg_manager.config.rank_size);
        MPI_Abort(comm, 1);
    }

    if (world_rank != cfg_manager.config.rank_idx) {
        fmt::print(stderr, "[deform] world_rank={} != rank_idx={}.\n", world_rank,
                             cfg_manager.config.rank_idx);
        MPI_Abort(comm, 1);
    }
    ...
    // Stretch relative to current box dimensions so sequential calls accumulate
    // deformation.
    ...
    if (cfg_manager.config.rank_idx == 0) {
        ...
        for (int i = 0; i < N_global; ++i) {
            h_particles[i].pos.x =
                    pbc_wrap_hd(h_particles[i].pos.x + 0.5 * dLx, Lx_new);
            h_particles[i].pos.y =
                    pbc_wrap_hd(h_particles[i].pos.y + 0.5 * dLy, Ly_new);
        }
    ...
    // Redistribute particles across ranks and rebuild device state for the new
    // box.
    distribute_particles_h2d();
    update_halo();
    cal_forces();
...
bool MDSimulation::check_eqlibrium(double sensitivity) {
    ...
    const double interval =
            (record_interval_dt > 0.0)
                    ? record_interval_dt
                    : (cfg_manager.config.dt > 0.0 ? cfg_manager.config.dt : 1.0);
    const double epsilon = 1e-12;

    // Collect global U on the record interval boundary so rank 0 can buffer it
...
void _build_interface_coords(const std::vector<Particle> &h_particles,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int ix = 0; ix < 3; ++ix) {
            ...
            // Check X bounds
            if (nx < x_min_limit || nx > x_max_limit)
                continue;
...
MDSimulation::triangulation_plot(bool is_plot, const std::string &filename,
                                                                 const std::string &csv_path) {
    if (cfg_manager.config.rank_idx != 0) {
        return std::nullopt;
    }
    ...
    for (int i = 0; i < n_particles; ++i) {
        ...
        if (x0 < w)
            add_vertex(i, x0 + Lx, y0);
        if (x0 > Lx - w)
            add_vertex(i, x0 - Lx, y0);

        // Periodic images near boundaries in Y
        if (y0 < w)
            add_vertex(i, x0, y0 + Ly);
        if (y0 > Ly - w)
            add_vertex(i, x0, y0 - Ly);

        // Corner combinations
        if (x0 < w && y0 < w)
            add_vertex(i, x0 + Lx, y0 + Ly);
        if (x0 < w && y0 > Ly - w)
            add_vertex(i, x0 + Lx, y0 - Ly);
        if (x0 > Lx - w && y0 < w)
            add_vertex(i, x0 - Lx, y0 + Ly);
    ...
    if (coords.size() < 6) {
        if (is_plot) {
            RankZeroPrint("[Warning] Not enough points for triangulation ({} "
                                        "points). Skipping.\n",
                                        coords.size() / 2);
        }
        return std::nullopt;
    ...
    try {
        ...
        for (const Tri &t : output.triVec) {
            ...
            if (!(in_base_box(x0, y0) || in_base_box(x1, y1) ||
                        in_base_box(x2, y2))) {
                continue;
            }
        ...
        if (is_plot) {
            plot_triangulation_python_from_triangles(h_particles, triangles_xy,
                                                                                             filename, csv_path, Lx, Ly,
                                                                                             cfg.SIGMA_AA, cfg.SIGMA_BB);
        }
        ...
        RankZeroPrint(
                "[Warning] GPU triangulation failed. Skipping plot. Reason: {}\n",
                e.what());
        return std::nullopt;
    }
}
...
ABPairNetworks MDSimulation::get_AB_pair_network(const TriangulationResult &tri,
    ...
    for (const auto &tri_idx : tri.triangles) {
        ...
        if (v0 < 0 || v1 < 0 || v2 < 0 || v0 >= static_cast<int>(vert_count) ||
...
namespace {
void plot_cwa_python(const std::string &csv_path,
    ...
    const std::string command =
            "~/.conda/envs/py3/bin/python ./python/plot_cwa_python.py --csv_path \"" +
            csv_path + "\" --output \"" + figure_path + "\"";
...
void MDSimulation::do_CWA_instant(int q_min, int q_max,
                                                                    const std::string &csv_path,
                                                                    const std::string &plot_path, bool is_plot,
                                                                    int step, bool is_LG) {
    if (cfg_manager.config.rank_idx != 0) {
        return;
    }
    ...
    std::vector<double> mode_accum(static_cast<size_t>(max_mode - q_min + 1),
                                                                 0.0);
    ...
    for (const auto &path : interface_paths) {
        ...
        double mean_x = std::accumulate(centered.begin(), centered.end(), 0.0) /
                                        centered.size();
        ...
        bool has_variation =
                std::any_of(centered.begin(), centered.end(),
        ...
        for (int mode = q_min; mode <= max_mode; ++mode) {
            ...
            for (int j = 0; j < n_grid_y; ++j) {
                ...
                accum += std::complex<double>(std::cos(phase), std::sin(phase)) *
                                 centered[j];
            }
    ...
    for (int mode = q_min; mode <= max_mode; ++mode) {
        ...
        double avg_mag = mode_accum[static_cast<size_t>(mode - q_min)] /
                                         static_cast<double>(n_interfaces_used);
    ...
    if (denominator <= 1e-16) {
        RankZeroPrint(
                "[CWA] Unable to determine slope for gamma (degenerate q-range).\n");
        return;
    }

    const double gamma = numerator / denominator;
    const double C0 =
            mean_y - gamma * mean_x; // intercept to absorb short-wavelength bias

    if (gamma <= 0.0) {
        RankZeroPrint(
                "[CWA] Non-positive gamma from regression; skipping output.\n");
        return;
    }

    fmt::memory_buffer buffer;
    fmt::format_to(std::back_inserter(buffer),
                                 "step, {}, gamma, {}, C0, {}, Ly, {}", step, gamma, C0, Ly);
    for (size_t idx = 0; idx < modes_used.size(); ++idx) {
        const int mode = modes_used[idx];
        fmt::format_to(std::back_inserter(buffer), ", q_sq_{}, {}", mode,
                                     q_sq[idx]);
        fmt::format_to(std::back_inserter(buffer), ", Cq_{}, {}", mode, c_q[idx]);
        fmt::format_to(std::back_inserter(buffer), ", S_q_{}, {}", mode,
                                     spectrum[idx]);
        fmt::format_to(std::back_inserter(buffer), ", hq_sq_{}, {}", mode,
                                     spectrum[idx]);
    }
    fmt::format_to(std::back_inserter(buffer), "\n");
...
MDSimulation::compute_interface_paths(int n_grid_y, double smoothing_sigma) {
    ...
    // Smoothing primarily along X removes density jumps perpendicular to
    // interface (Smoothing along Y removes waves - use carefully!)
    ...
    for (int x = 0; x < n_grid_x; ++x) {
        for (int y = 0; y < n_grid_y; ++y)
            profile_1d[x] += temp_grid[y * n_grid_x + x];
    }
    ...
    for (int k = 0; k < (int)anchors.size(); ++k) {
            ...
            for (int x = 0; x < n_grid_x; ++x) {
                ...
                if (v1 * v2 <= 0.0 && v1 != v2) {
                    ...
                    if (std::abs(dist) < Lx / 4.0) {
                        // We accumulate the RELATIVE distance (dx) to avoid PBC averaging
                        // bugs (e.g. averaging x=0.1 and x=L-0.1 should give x=0.0, not
                        // L/2)
        ...
        if (start_idx == -1) {
            ...
            // Forward fill + Wrap
            // We iterate 2*N times to ensure we cover gaps crossing the periodic
            // boundary
            ...
            for (int i = 0; i < n_grid_y; ++i) {
                ...
                if (valid[idx])
                    last_val = final_path_x[idx];
                else
                    final_path_x[idx] = last_val; // Simple hold interpolation
            }
...
std::vector<std::vector<double>>
MDSimulation::compute_interface_paths_LG(int n_grid_y, double smoothing_sigma) {
    std::vector<std::vector<double>> result;
    if (cfg_manager.config.rank_idx != 0)
        return result;
    ...
    auto choose_two_crossings =
            [&](const std::vector<double> &crossings) -> std::array<double, 2> {
        if (crossings.empty()) {
            return {0.0, Lx * 0.5};
        }
        if (crossings.size() == 1) {
    ...
    auto fallback_from_gradient =
            [&](const double *row) -> std::array<double, 2> {
        std::vector<std::pair<double, double>>
                grads; // (gradient magnitude, x-position)
    ...
    for (int y = 0; y < n_grid_y; ++y) {
        ...
        if (crossings.size() >= 2) {
            ...
            // We assume dense phase is in the middle of the box and gas on both
            // edges.
        ...
        if (y == 0) {
            ...
            // Maintain continuity by matching to previous row with minimal circular
            // distance.
            double prev0 = result[0][y - 1];
            double prev1 = result[1][y - 1];

            double cost_direct = circular_distance(prev0, interfaces[0]) +
                                                     circular_distance(prev1, interfaces[1]);
            double cost_swap = circular_distance(prev0, interfaces[1]) +
                                                 circular_distance(prev1, interfaces[0]);
...
std::vector<std::vector<double>>
MDSimulation::get_smooth_interface(int n_grid_y, double smoothing_sigma) {
...
std::vector<std::vector<double>>
MDSimulation::get_smooth_interface_LG(int n_grid_y, double smoothing_sigma) {
    std::vector<std::vector<double>> paths =
...
std::vector<double> MDSimulation::get_pressure_profile(int n_bins_local) {
    ...
    MPI_Reduce(local_Pxx.data(), world_rank == 0 ? global_Pxx.data() : nullptr,
                         n_bins_local, MPI_DOUBLE, MPI_SUM, 0, comm);
    MPI_Reduce(local_Pyy.data(), world_rank == 0 ? global_Pyy.data() : nullptr,
                         n_bins_local, MPI_DOUBLE, MPI_SUM, 0, comm);
    MPI_Reduce(local_Pxy.data(), world_rank == 0 ? global_Pxy.data() : nullptr,
                         n_bins_local, MPI_DOUBLE, MPI_SUM, 0, comm);
...
std::vector<int> MDSimulation::get_N_profile(int n_bins_per_rank) {
    ...
    size_t shared_mem_size =
            static_cast<size_t>(2 * n_bins_per_rank * sizeof(int));
    ...
    if (blocks > 0) {
        local_density_profile_kernel<<<blocks, threads, shared_mem_size>>>(
                thrust::raw_pointer_cast(d_particles.data()), n_local, n_bins_per_rank,
                xmin, xmax, thrust::raw_pointer_cast(d_count_A.data()),
    ...
    MPI_Gather(count_A.data(), local_bins, MPI_INT,
                         world_rank == 0 ? all_A.data() : nullptr, local_bins, MPI_INT, 0,
                         comm);
    MPI_Gather(count_B.data(), local_bins, MPI_INT,
                         world_rank == 0 ? all_B.data() : nullptr, local_bins, MPI_INT, 0,
                         comm);
    ...
    if (total_bins > 0) {
        MPI_Bcast(result.data(), total_bins * 2, MPI_INT, 0, comm);
    }
...
std::vector<std::vector<double>>
MDSimulation::locate_interface(const delaunator::Delaunator &d) {

    std::vector<std::vector<double>> result;

    if (cfg_manager.config.rank_idx != 0 || d.triangles.empty() ||
            vertex_to_idx.empty()) {
        return result;
    }
    ...
    for (size_t i = 0; i < num_triangles; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            if (processed_edges.count({v1, v2}))
                continue;
...
void MDSimulation::save_env(const std::string &filename, const int step) {
    ...
    if (cfg_manager.config.rank_idx == 0) {
        ...
        // h_particles is assumed to hold all particles on Rank 0 (gathered via
        // sample_collect)
