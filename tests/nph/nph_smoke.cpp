...
int main() {
    const std::filesystem::path cfg_path = "./tests/nph/config.json";
    ...
    if (cfg_mgr.config.rank_size != rank_size) {
        if (rank_idx == 0) {
            fmt::print(stderr,
                       "[nph_smoke] rank size = {} doesn't match config.\n",
                       rank_size);
        }
        MPI_Finalize();
        return 1;
    ...
    for (int step = 0; step < auto_steps; ++step) {
        ...
        if (!std::isfinite(p_inst)) {
            if (rank_idx == 0) {
                fmt::print(stderr,
                           "[nph_smoke] non-finite pressure during AUTO_TARGET at step {}\n",
                           step);
            }
            MPI_Finalize();
            return 2;
    ...
    for (int step = 0; step < production_steps; ++step) {
        sim.step_single_NPH(target_pressure);
        ...
        if (!std::isfinite(p_inst) || !std::isfinite(sim.get_Lx()) ||
            ...
            if (rank_idx == 0) {
                fmt::print(stderr,
                           "[nph_smoke] invalid state in NPH at step {}: "
                           "Lx={}, Ly={}, P={}\n",
    ...
    if (rank_idx == 0) {
        fmt::print("[nph_smoke] completed. Lx={}, Ly={}, P_last={}\n",
