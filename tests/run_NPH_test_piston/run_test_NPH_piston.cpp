...
const char *phase_name(Phase p) {
    ...
    case Phase::AUTO_TARGET:
        return "AUTO_TARGET";
    case Phase::NPH_PISTON_WARMUP:
        return "NPH_PISTON_WARMUP";
    case Phase::NPH_PISTON_PRODUCTION:
        return "NPH_PISTON_PRODUCTION";
    default:
        return "UNKNOWN";
    }
}
...
int main(int argc, char **argv) {
    const std::string cfg_path = (argc > 1)
                                         ? argv[1]
                                         : "./tests/run_NPH_test_piston/config_piston.json";
    ...
    if (cfg_mgr.config.rank_size != rank_size) {
        if (rank_idx == 0) {
            fmt::print(stderr,
                       "[run_test_NPH_piston] rank size = {} doesn't match config.\n",
                       rank_size);
        }
        MPI_Finalize();
        return 1;
    ...
    const int n_record_interval = std::max(
            1, static_cast<int>(cfg_mgr.config.save_dt_interval / cfg_mgr.config.dt));
    ...
    const fs::path base_dir = "./tests/run_NPH_test_piston";
    ...
    const fs::path saved_cfg_path = saved_env_dir / "config_piston.json";
    const fs::path pressure_csv = sample_csv_dir / "pressure_log_piston.csv";
    const fs::path piston_csv = sample_csv_dir / "piston_state_log.csv";
    const fs::path thermo_csv = sample_csv_dir / "thermo_log_piston.csv";
    ...
    const std::string tag = "run_test_NPH_piston";

    if (rank_idx == 0) {
        std::ofstream clear_pressure(pressure_csv, std::ios::out | std::ios::trunc);
        std::ofstream clear_piston(piston_csv, std::ios::out | std::ios::trunc);
        std::ofstream clear_thermo(thermo_csv, std::ios::out | std::ios::trunc);

        if (!clear_pressure || !clear_piston || !clear_thermo) {
            fmt::print(stderr,
                       "[run_test_NPH_piston] Failed to initialize output CSV files under {}\n",
                       sample_csv_dir.string());
            MPI_Abort(MPI_COMM_WORLD, 4);
        }

        append_csv(pressure_csv, rank_idx, tag,
                   "step,t,phase,P_inst,P_target,rolling_mean,rolling_std,rel_err,Lx,Ly,area,dLy_dt,F_piston\n");
        append_csv(piston_csv, rank_idx, tag,
                   "step,t,phase,Lx,Ly,area,dLy_dt,a_piston,F_piston,P_inst,P_target\n");
        append_csv(thermo_csv, rank_idx, tag,
                   "step,t,phase,U_tot,K_tot,P_inst,P_target,Lx,Ly,area,F_piston\n");
    ...
    for (int step = 0; step < hard_step_limit && !finished; ++step) {
        ...
        if (phase_used == Phase::AUTO_TARGET) {
            ...
            sim.step_single_NPH_piston(P_target);
        ...
        if (rank_idx == 0) {
            if (phase_used == Phase::AUTO_TARGET) {
                ++auto_steps_done;
            } else if (phase_used == Phase::NPH_PISTON_WARMUP) {
                ++warmup_steps_done;
            } else {
                ++nph_steps_done;
            }
            ...
            if (phase_used == Phase::AUTO_TARGET &&
                ...
                stable_windows = 0;
                fmt::print(
                        "[run_test_NPH_piston] AUTO_TARGET complete at step {}. P_target={}\n",
                        step, P_target);
            }

            if (phase_used == Phase::NPH_PISTON_WARMUP) {
                if (warmup_steps_done >= warmup_min_steps &&
                    ...
                    if (stable_windows >= stable_required) {
                        phase = Phase::NPH_PISTON_PRODUCTION;
                        fmt::print(
                                "[run_test_NPH_piston] piston warmup complete at step {}\n",
                                step);
                    }
                }

                if (phase == Phase::NPH_PISTON_WARMUP &&
                    warmup_steps_done >= warmup_max_steps) {
                    phase = Phase::NPH_PISTON_PRODUCTION;
                    fmt::print(
                            "[run_test_NPH_piston] Forced warmup->production switch at step {}\n",
                            step);
                }
            }

            const double area = sim.get_Lx() * sim.get_Ly();
            const double dLy_dt = (phase_used == Phase::AUTO_TARGET)
                                          ? 0.0
                                          : sim.get_nph_piston_velocity();
            const double F_piston = (phase_used == Phase::AUTO_TARGET)
                                            ? 0.0
                                            : sim.get_nph_piston_force();
            const double a_piston =
                    (phase_used == Phase::AUTO_TARGET)
                            ? 0.0
                            : (F_piston - P_target * sim.get_Lx()) /
            ...
            if (step % eval_interval == 0) {
                ...
                append_csv(pressure_csv, rank_idx, tag,
                           "{},{:.6f},{},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e}\n",
                           step, sim.t, phase_name(phase_used), P_inst, P_target,
                           stats.mean, stats.stddev, rel_err, sim.get_Lx(),
                           sim.get_Ly(), area, dLy_dt, F_piston);

                append_csv(piston_csv, rank_idx, tag,
                           "{},{:.6f},{},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e},{:.10e}\n",
                           step, sim.t, phase_name(phase_used), sim.get_Lx(),
                           sim.get_Ly(), area, dLy_dt, a_piston, F_piston, P_inst,
                           P_target);
            }

            if (do_progress_log) {
                fmt::print(
                        "[run_test_NPH_piston] step={} t={:.6f} phase={} Lx={:.10e} Ly={:.10e} "
                        "dLy_dt={:.10e} F_piston={:.10e} P_inst={:.10e} P_target={:.10e} U_tot={:.10e} K_tot={:.10e}\n",
                        step, sim.t, phase_name(phase_used), sim.get_Lx(),
                        sim.get_Ly(), dLy_dt, F_piston, P_inst, P_target, U_tot,
                        K_tot);
            }
            ...
            if (do_record) {
                append_csv(thermo_csv, rank_idx, tag,
                           ...
                           P_target, sim.get_Lx(), sim.get_Ly(), area, F_piston);
                ...
                if (step % snapshot_interval == 0) {
                    ...
                    const fs::path frame_svg =
                            frame_dir / ("frame_nph_piston_step_" + step_str + ".svg");
                    const fs::path frame_csv = frame_csv_dir /
                                               ("frame_nph_piston_step_" + step_str +
                                                ".csv");
                    ...
                    try {
                        ...
                        fmt::print(stderr,
                                   "[run_test_NPH_piston] Failed to plot frame at step {}: {}\n",
                                   step, e.what());
            ...
            if (phase_used == Phase::NPH_PISTON_PRODUCTION &&
    ...
    if (rank_idx == 0) {
        fmt::print(
                "[run_test_NPH_piston] Finished. phase={}, Lx={}, Ly={}, P_last={}\n",
